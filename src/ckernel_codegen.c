...
{
    if (!spec || spec->role != CK_ROLE_WEIGHT || !spec->name) {
        return 0;
    }

    /* Weight-only quantization targets: big GEMM weights and tied embeddings.
     * Small vectors (norm scales, biases) remain fp32 for simplicity. */
    return (strcmp(spec->name, "token_emb") == 0) ||
           (strcmp(spec->name, "wq") == 0) ||
           (strcmp(spec->name, "wk") == 0) ||
           (strcmp(spec->name, "wv") == 0) ||
           (strcmp(spec->name, "wo") == 0) ||
           (strcmp(spec->name, "w1") == 0) ||
           (strcmp(spec->name, "w2") == 0) ||
           (strcmp(spec->name, "lm_head_weight") == 0);
}
...
static void emit_bump_bytes_assignment_weight_dtype(FILE *out,
                                                    ...
                                                    const CKDimToken *shape)
{
    fprintf(out, "%sif (m->weight_dtype == CK_DT_Q4_K) {\n", indent);
    fprintf(out, "%s    %s%s_offset = bump_bytes(&off, ck_dtype_row_bytes(CK_DT_Q4_K, (",
            indent, struct_prefix, name);
    emit_shape_expr(out, shape);
    fprintf(out, ")), CACHELINE_BYTES);\n");
    fprintf(out, "%s} else {\n", indent);
    fprintf(out, "%s    %s%s_offset = bump_bytes(&off, (", indent, struct_prefix, name);
    emit_shape_expr(out, shape);
    fprintf(out, ") * elem_bytes, CACHELINE_BYTES);\n");
...
static void emit_global_allocations(FILE *out)
{
    for (size_t i = 0; i < ck_decoder_buffer_count; ++i) {
        ...
        if (ck_buffer_uses_weight_dtype(spec)) {
            emit_bump_bytes_assignment_weight_dtype(out, "    ", "m->", spec->name, spec->shape);
            continue;
        }
...
static void emit_layer_allocations(FILE *out)
{
    for (size_t i = 0; i < ck_decoder_buffer_count; ++i) {
        ...
        if (ck_buffer_uses_weight_dtype(spec)) {
            emit_bump_bytes_assignment_weight_dtype(out, "        ", "L->", spec->name, spec->shape);
            continue;
        }
...
static int emit_runtime_preamble(FILE *out)
{
    ...
    fprintf(out,
            "static inline uint8_t *ptr_u8(uint8_t *base, size_t offset) {\n"
            "    return base + offset;\n"
            "}\n"
            "static inline const void *cptr_void(const uint8_t *base, size_t offset) {\n"
...
static void emit_library_api(FILE *out, const CKIRGraph *forward)
{
    fprintf(out,
            ...
            "static int g_fuse_swiglu_decode = -2;\n"
            "static int g_fuse_attn_decode = -2;\n\n"
            "static int ck_fuse_swiglu_decode_mode(void)\n"
            "{\n"
            "    if (g_fuse_swiglu_decode != -2) return g_fuse_swiglu_decode;\n"
            "    const char *env = getenv(\"CK_FUSE_SWIGLU_DECODE\");\n"
            "    if (!env || !env[0]) {\n"
            "        g_fuse_swiglu_decode = -1; /* auto */\n"
            "        return g_fuse_swiglu_decode;\n"
            "    }\n"
            "    if (env[0] == '0' || env[0] == 'n' || env[0] == 'N' || env[0] == 'f' || env[0] == 'F') {\n"
            "        g_fuse_swiglu_decode = 0;\n"
            "    } else {\n"
            "        g_fuse_swiglu_decode = 1;\n"
            "    }\n"
            "    return g_fuse_swiglu_decode;\n"
            "}\n\n");

    fprintf(out,
            "static int ck_fuse_attn_decode_mode(void)\n"
            "{\n"
            "    if (g_fuse_attn_decode != -2) return g_fuse_attn_decode;\n"
            "    const char *env = getenv(\"CK_FUSE_ATTN_DECODE\");\n"
            "    if (!env || !env[0]) {\n"
            "        g_fuse_attn_decode = -1; /* auto */\n"
            "        return g_fuse_attn_decode;\n"
            "    }\n"
            "    if (env[0] == '0' || env[0] == 'n' || env[0] == 'N' || env[0] == 'f' || env[0] == 'F') {\n"
            "        g_fuse_attn_decode = 0;\n"
            "    } else {\n"
            "        g_fuse_attn_decode = 1;\n"
            "    }\n"
            "    return g_fuse_attn_decode;\n"
            "}\n\n");
    ...
    fprintf(out,
            ...
            "    int fuse_swiglu_mode = ck_fuse_swiglu_decode_mode();\n"
            "    int use_fused_swiglu = 0;\n"
            "    if (fuse_swiglu_mode > 0) {\n"
            "        use_fused_swiglu = 1;\n"
            "    } else if (fuse_swiglu_mode == 0) {\n"
            "        use_fused_swiglu = 0;\n"
            "    } else if (!ck_strict_parity_enabled()) {\n"
            "        use_fused_swiglu = 1;\n"
            "    }\n"
            "    int fuse_attn_mode = ck_fuse_attn_decode_mode();\n"
            "    int use_fused_attn = 0;\n"
            "    if (fuse_attn_mode > 0) {\n"
            "        use_fused_attn = 1;\n"
            "    } else if (fuse_attn_mode == 0) {\n"
            "        use_fused_attn = 0;\n"
            "    } else if (!ck_strict_parity_enabled()) {\n"
            "        use_fused_attn = 1;\n"
            "    }\n"
            "    if (m->weight_dtype != CK_DT_FP32) {\n"
            "        use_fused_swiglu = 0;\n"
            "        use_fused_attn = 0;\n"
            "    }\n"
            "\n"
            ...
            "\n"
            "            ck_layer_forward_rmsnorm_swiglu_decode_q4_k(&p, t, cache_cap);\n"
            "        } else {\n"
            ...
            "            if (use_fused_attn) {\n"
            "                if (use_fused_swiglu) {\n"
            "                    ck_layer_forward_rmsnorm_swiglu_decode_fused_attn_mlp(&p, t, cache_cap);\n"
            "                } else {\n"
            "                    ck_layer_forward_rmsnorm_swiglu_decode_fused_attn(&p, t, cache_cap);\n"
            "                }\n"
            "            } else if (use_fused_swiglu) {\n"
            "                ck_layer_forward_rmsnorm_swiglu_decode_fused(&p, t, cache_cap);\n"
            "            } else {\n"
    ...
    fprintf(out,
            ...
            "    }\n"
            "    if (g_model.training_enabled && g_model.weight_dtype == CK_DT_Q4_K) {\n"
            "        fprintf(stderr, \"Q4_K weights are inference-only; disable training for this model\\n\");\n"
            "        return -5;\n"
            "    }\n"
...
int ck_codegen_emit_runtime(const CKIRGraph *forward, const char *path, CKEmitMode mode)
{
    ...
    fprintf(out,
            ...
            "    if (m->weight_dtype == CK_DT_Q4_K) {\n"
            "        /* Q4_K requires the K dimension to be a multiple of 256 weights. */\n"
            "        if ((m->aligned_embed_dim %% 256) != 0) return -1;\n"
            "        if ((aligned_intermediate_dim %% 256) != 0) return -1;\n"
            "        /* Output projection expects concatenated heads to match aligned_embed_dim. */\n"
            "        if ((size_t)m->num_attention_heads * m->aligned_head_dim != m->aligned_embed_dim) return -1;\n"
            "    }\n"
            "\n"
    ...
    fprintf(out,
            ...
            "    int q4k = (m->weight_dtype == CK_DT_Q4_K);\n"
            "\n"
            "    size_t tok_elems = (size_t)m->vocab_size * m->aligned_embed_dim;\n"
            "    if (q4k) {\n"
            "        if (read_bytes_file(f, ptr_u8(base, m->token_emb_offset),\n"
            "                           ck_dtype_row_bytes(CK_DT_Q4_K, tok_elems)) != 0) goto fail;\n"
            "    } else {\n"
            "        if (read_floats_file(f, ptr_f32(base, m->token_emb_offset), tok_elems) != 0) goto fail;\n"
            "    }\n"
            ...
            "        if (q4k) {\n"
            "            if (read_bytes_file(f, ptr_u8(base, L->wq_offset), ck_dtype_row_bytes(CK_DT_Q4_K, q_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (read_bytes_file(f, ptr_u8(base, L->wk_offset), ck_dtype_row_bytes(CK_DT_Q4_K, kv_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (read_bytes_file(f, ptr_u8(base, L->wv_offset), ck_dtype_row_bytes(CK_DT_Q4_K, kv_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (read_bytes_file(f, ptr_u8(base, L->wo_offset), ck_dtype_row_bytes(CK_DT_Q4_K, wo_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (read_bytes_file(f, ptr_u8(base, L->w1_offset), ck_dtype_row_bytes(CK_DT_Q4_K, w1_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (read_bytes_file(f, ptr_u8(base, L->w2_offset), ck_dtype_row_bytes(CK_DT_Q4_K, w2_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "    size_t tok_elems = (size_t)m->vocab_size * m->aligned_embed_dim;\n"
            "    if (q4k) {\n"
            "        if (write_bytes_file(f, ptr_u8(base, m->token_emb_offset),\n"
            "                            ck_dtype_row_bytes(CK_DT_Q4_K, tok_elems)) != 0) goto fail;\n"
            "    } else {\n"
            "        if (write_floats_file(f, ptr_f32(base, m->token_emb_offset), tok_elems) != 0) goto fail;\n"
            "    }\n"
            ...
            "        if (q4k) {\n"
            "            if (write_bytes_file(f, cptr_void(base, L->wq_offset), ck_dtype_row_bytes(CK_DT_Q4_K, q_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (write_bytes_file(f, cptr_void(base, L->wk_offset), ck_dtype_row_bytes(CK_DT_Q4_K, kv_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (write_bytes_file(f, cptr_void(base, L->wv_offset), ck_dtype_row_bytes(CK_DT_Q4_K, kv_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (write_bytes_file(f, cptr_void(base, L->wo_offset), ck_dtype_row_bytes(CK_DT_Q4_K, wo_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (write_bytes_file(f, cptr_void(base, L->w1_offset), ck_dtype_row_bytes(CK_DT_Q4_K, w1_w)) != 0) goto fail;\n"
            "        } else {\n"
            ...
            "        if (q4k) {\n"
            "            if (write_bytes_file(f, cptr_void(base, L->w2_offset), ck_dtype_row_bytes(CK_DT_Q4_K, w2_w)) != 0) goto fail;\n"
            "        } else {\n"
